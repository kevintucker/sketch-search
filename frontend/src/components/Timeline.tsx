    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="cyber-export-button" onClick={async () => {
          ...
          const resp = await fetch(`${API_BASE}/api/export/clip`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ video_url: (videoUrl || '').replace(API_BASE,'/data'), start_s, duration_s, shot_id: pipeline.shotId || 'clip' }),
          });
          if (!resp.ok) return;
