...
export const Dashboard: React.FC = () => {
  ...
  const uploadToBackend = async (file: File) => {
    try {
      ...
      const vids = (data.video_urls || []).map((u: string) => `${API_BASE}${u}`);
      setVideoUrls(vids);
      setVideoUrl(vids[0] || null);
      setPipeline({
        shotId: data.shot_id,
        sceneGraphUrl: data.scene_graph_url ? `${API_BASE}${data.scene_graph_url}` : null,
        motionPlanUrl: data.motion_plan_url ? `${API_BASE}${data.motion_plan_url}` : null,
        keyframesUrl: data.keyframes_url ? `${API_BASE}${data.keyframes_url}` : null,
        videoUrl: vids[0] || null,
      });
      if (data.scene_graph_url) {
        const sgResp = await fetch(`${API_BASE}${data.scene_graph_url}`);
        const sg = await sgResp.json();
        setSceneGraph(sg);
      }
      if (data.motion_plan_url) {
        const mpResp = await fetch(`${API_BASE}${data.motion_plan_url}`);
        const mp = await mpResp.json();
        setMotionPlan(mp);
      }
      if (data.shot_id) {
        const kfResp = await fetch(`${API_BASE}/api/keyframes/${data.shot_id}`);
        const kf = await kfResp.json();
        const arr = (kf.items || []).map((x: string) => `${API_BASE}${x}`);
        setKeyframes(arr);
          ...
          {sceneGraph && (
            <div className="mt-4">
              ...
              <button className="cyber-button mt-2" onClick={async () => {
                ...
                await fetch(`${API_BASE}/api/scene_graph/${pipeline.shotId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        ...
        {keyframes.length > 0 && (
          <div className="mt-4">
            ...
            <button className="cyber-button mt-3" onClick={async () => {
              ...
              await fetch(`${API_BASE}/api/qa/canonical_frame`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_id: 'demo', shot_id: pipeline.shotId, frame_url: selectedFrame }) });
